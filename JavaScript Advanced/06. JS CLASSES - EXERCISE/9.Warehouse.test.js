...
describe.only("Warehouse", function () {
    ...
    describe("constructor", function () {
        ...
        it('with number param should pass correct', function () {
            // wh = new Warehouse(1);
            wh.capacity = 0;
            expect(wh.capacity).to.be.equal(0, "The defaut capacity is wrong!")
        })
    ...
    describe("orderProducts", function () {
        ...
        it('with smaller than capacity, quantity should sort "Food" correct', function () {
            ...
            expect(wh.orderProducts('Food').toString()).to.be.equal(result.toString(), "The added product are not sorted!");
        })
        it('with smaller than capacity, quantity should sort "Drink" correct', function () {
            ...
            expect('' + wh.orderProducts('Drink')).to.be.equal('' + result, "The added product are not sorted!");
        })
        it('with equal to 0 quantity should sort "Drink" correct', function () {
            ...
            expect('' + wh.orderProducts('Food')).to.be.equal('' + result, "The added product are not sorted!");
        })
    ...
    describe('revision', function () {
        ...
        //     wh.addProduct('Food', 'Cucumbers', 2);
        //     wh.addProduct('Food', 'Tomatos', 5);
        //     // console.log(wh.availableProducts)
        //     // console.log(wh.orderProducts('Food'))
        //     // console.log(wh.occupiedCapacity())
        //     let result = resultStr(wh.availableProducts, wh.occupiedCapacity);
        //     expect(wh.revision().toString()).to.be.equal(resul.toString(), "The result string is wrong!")
    ...
    describe("scrapeAProduct", function () {
        ...
        it('with existing product and smaler quantity should pass correct', function () {
            ...
            wh.addProduct('Food', 'Tomatos', 5);
            wh.addProduct('Food', 'Apples', 5);
            expect(wh.scrapeAProduct('Tomatos', 3)).to.deep.equal(result, "The added quantity is bigger than capacity!");
        })
        it('with existing product and bigger quantity should pass correct', function () {
            ...
            wh.addProduct('Food', 'Tomatos', 5);
            wh.addProduct('Food', 'Apples', 5);
            expect(wh.scrapeAProduct('Apples', 6)).to.deep.equal(result, "The added quantity is bigger than capacity!");
        })
        it('with existing product and bigger quantity should pass correct', function () {
            // wh = new Warehouse(20);
            let result = { Juce: 0 };
            wh.addProduct('Drink', 'Juce', 3);
            expect(wh.scrapeAProduct('Juce', 50)).to.deep.equal(result, "The added quantity is bigger than capacity!");
        })
        it('when try to add bigger than capacity quantity should trow error', function () {
            // wh = new Warehouse(20);
            wh.addProduct('Drink', 'Juce', 3);
