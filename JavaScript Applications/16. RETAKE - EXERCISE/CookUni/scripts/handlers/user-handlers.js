...
handlers.registerUser = function (ctx) {
    let { firstName, lastName, username, password, rePassword } = ctx.params;

    if (!firstName || firstName.length < 2) {
        notifications.showError("The first name should be at least 2 characters long!")
        return;
    }
    if (!lastName || lastName.length < 2) {
        notifications.showError("The last name should be at least 2 characters long!")
        return;
    }
    if (!username || username.length < 3) {
        notifications.showError("The username should be at least 3 characters long!")
